...
const handleSyncedCalendarSubscription = async () => {
  ...
  // Step 1: Identify ALL potential candidates from SyncedToCalendar for supported integrations
  const tobeWatched = await prisma.syncedToCalendar.findMany({
    where: {
      subscription: null,
    },
  });
  ...
  if (!tobeWatched.length) {
    log.info("No SyncedToCalendar records found needing subscription management.");
    return [];
  }
  ...
  const results = await Promise.allSettled(
    tobeWatched.map(async (calendar) => {
      ...
      if (!credentialId) {
        log.error(`SyncedToCalendar record ${syncedCalendarId} missing credentialId despite query filter.`);
      ...
      try {
        ...
        // Link it back to the SyncedToCalendar record
        await prisma.syncedToCalendar.update({
